//circle
    ...
    const textSpan = document.querySelector('.flip-text');
        const texts = ['Web Developer', 'UI Designer', 'UX Researcher', 'Product Designer'];
        let index = 0;
...
slideInterval = setInterval(slideRight, 190000);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function toggleNavLinks() {
    if (window.innerWidth <= 768) {
      navLinks.style.display = checkbox.checked ? 'flex' : 'none';
    }
  }
  ...
  checkbox.addEventListener('change', toggleNavLinks);
